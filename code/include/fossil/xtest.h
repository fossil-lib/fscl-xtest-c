...
// Typedef to simplify the usage of the struct cdna
...
typedef enum {
    ...
    CDNA_OPT_BIT_HIGH,
    CDNA_OPT_BIT_NOT_HIGH,
    CDNA_OPT_BIT_LOW,
    CDNA_OPT_BIT_NOT_LOW,
    CDNA_OPT_WITHIN
} cdna_opt;
...
#define xtest_as_int(value) CDNA_ASSIGN(CDNA_INT_TYPE, &(cdna_data){ .int64_type = value })
...
// Helper function to assign invalid values
#define xtest_as_invalid() CDNA_ASSIGN(CDNA_INVALID_TYPE, &(cdna_data){})

// Helper function to assign unknown values
#define xtest_as_unknown() CDNA_ASSIGN(CDNA_UNKNOWN_TYPE, &(cdna_data){})
...
// Macro to assert that the elapsed time, given in nanoseconds, does not exceed a specified maximum time in seconds.
// It is intended for use in test scenarios where a strict upper limit on execution time is crucial.
#define XMARK_ASSUME_SECONDS(elapsed_time_ns, max_seconds) xmark_assert_seconds(elapsed_time_ns, max_seconds)

// Macro to assert that the elapsed time, given in nanoseconds, does not exceed a specified maximum time in minutes.
// Similar to XMARK_ASSERT_SECONDS but with the time limit specified in minutes.
#define XMARK_ASSUME_MINUTES(elapsed_time_ns, max_minutes) xmark_assert_minutes(elapsed_time_ns, max_minutes)
